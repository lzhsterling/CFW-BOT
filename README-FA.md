...
cd CFW-BOT
```
7. دسترسی لازم برای اجرا شدن فایل  requirement.sh را به آن بدهید.
```bash
...
9. فایل پایتون install.py را اجرا کنید، این فایل با پرسیدن مقادیر API و یوزر آیدی تنظیمات لازم را برای شروع اجرا بات به صورت خودکار انجام میدهد.
```bash
 python3 install.py
 ```
 10. حالا میتونید بات را اجرا کنید:
 ```bash
 python3 cfw.py
 ```
### نکته:
...
با ارسال این دستور برای چک کردن آپدیت‌های جدید کدها در مخزن و دریافت آن‌ها
...
با این دستور وارد فولدر ربات شده و مخزن را چک کرده در صورت وجود تغییرات جدید در هرکدام از فایل‌ها؛ آن ها را در سورس دستگاه اعمال کرده و در نهایت بات را اجرا میکند.
