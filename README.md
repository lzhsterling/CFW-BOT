...
> شما لطفا برید [از این مخزن](https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/addressesapi.txt) پروکسی آی‌پی بردارید و داخل ربات تو مرحله دوم که ازتون آی‌پی میخواد براش ارسال کنید، دقت کنید که توی لیست فقط از اون آی‌پی‌هایی بردارید که پورت 443 دارن، پورت های دیگه جواب نمیدن.برای مثال:
> `129.159.38.24:443#US`
> این آخرش نوشته 443 و بعدشم ک نوشته آمریکا، کشورش مهم نیست زیاد، فقط پورت `443` ، و اینکه اون لیست پروکسی آی‌پی روزی دوسه بار بروزرسانی میشه، حتی میتونی از این دامین `workers.bestip.one` بجای پروکسی آی‌پی استفاده کنی هنوز ک خوب کار میده، لوکیشن انگلیس، امریکا، لوکزامبورگ.
